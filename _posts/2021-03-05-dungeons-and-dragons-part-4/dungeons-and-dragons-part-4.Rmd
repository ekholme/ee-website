...
description: |
  Predicting monster challenge ratings
author:
  - name: EE
...
So, there are a number of things that stand out to me. First, it's clearly not a bad model, but there are some areas where the model is missing by quite a bit. Also, and I've mentioned this in earlier posts, but the Tarrasque and it's super-high CR is probably exerting a really strong influence on this model.
...
## Refitting with a Log-Transformed DV & Post-Processing
...
```{r}
lasso_coefs <- pull_workflow_fit(lasso_fin_fit$.workflow[[1]]) %>%
  vi()
...
One thing to keep in mind here is the reference categories for our size and type variables. For size, the reference is "Gargantuan," which is the largest type of monster, so it makes sense that every other size has a lower CR. For type, the reference is "beast." It's interesting that dragons tend to have lower CRs than beasts (all other things held constant), but there are also some junior dragons ("wyrmlings") in D&D, plus some things that aren't *really* dragons (i.e. "dragon turtles") that are classified as dragons by their type.

Another interesting result here is that strength is negatively associated with CR. This is because strength and constitution are so highly correlated, and so any contribution of strength is actually accounted for by constitution.
